...
class CIBlockPropertyCProp
{
    ...
    private static function checkHtmlInputs()
    {
        $result = [];
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'HTML_BITRIX') === 0) {
                $code = substr($key, 11);
                $result[] = [
                    'key' => $code,
                    'value' => $value
...
{
    $htmlInputs = self::checkHtmlInputs();
    if (!empty($htmlInputs)) {
        foreach ($htmlInputs as $item) {
            $arValue['VALUE'][$item['key']] = $item['value'];
        }
    }
    ...
    if (!$isEmpty) {
        $arResult['VALUE'] = json_encode($arValue['VALUE'], JSON_UNESCAPED_UNICODE);
    } else {
