...
// Если указан IBLOCK_ID работаем только с ним
if (!empty($iblockId) && $iblockId > 0) {
    $iblockIds = [(int)$iblockId];
} else {
    // Получаем все инфоблоки этого типа
    $rsIblock = CIBlock::GetList([], ["TYPE" => $iblockType, "ACTIVE" => "Y"]);
    $iblockIds = [];
...
// Получаем элементы из всех инфоблоков
$rsElement = CIBlockElement::GetList(
	...
	if (!empty($arResult['ITEMS']))
	{
		...
		// Группируем элементы по IBLOCK_ID
		if (!empty($arResult['ITEMS']) && is_array($arResult['ITEMS'])) {
			$arResult['ITEMS'] = IblockElementList::groupItemsByIblockId($arResult['ITEMS']);
		}
